...
public:
    ...
    void updatePlayerChunkTarget(unsigned short playerID, int numChunksLoaded);
};
...
void ServerWorld<integrated>::findChunksToLoad() {
    ...
    for (auto& [playerID, player] : m_players) {
        if (!player.allChunksLoaded() && (integrated || (player.getNumLoadedChunks() < player.getChunkTarget()))) {
            int chunkPosition[3];
...
template<bool integrated>
int ServerWorld<integrated>::loadChunkFromPacket(Packet<unsigned char, 9 * constants::CHUNK_SIZE *
    constants::CHUNK_SIZE * constants::CHUNK_SIZE>& payload, Position& chunkPosition) {
    ...
    // Synchronise the player's chunk record with which chunks have been loaded
    ServerPlayer& player = m_players[0];
    player.setChunkLoaded(chunkPosition);
    player.allChunksLoaded();
    // Update the server with how many chunks the player has built
    return player.getNumLoadedChunks();
}
...
template<bool integrated>
void ServerWorld<integrated>::updatePlayerChunkTarget(unsigned short playerID, int numChunksLoaded) {
    m_players.at(playerID).updateChunkTarget(numChunksLoaded);
}